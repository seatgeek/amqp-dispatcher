...
def validate(config_file):
    ...
    valid_handler = validate_startup_handler(config, logger)
    valid_consumers = validate_consumers(config, logger)
    if valid_handler and valid_consumers:
        return
    sys.exit(1)


def validate_startup_handler(config, logger):
    ...
    try:
        ...
        logger.error('[{0}] Invalid startup_handler: {1}'.format(
            startup_handler_str,
            e
        ))
        return False
    ...
    if handler is None:
        logger.error('[{0}] Invalid startup_handler'.format(
            startup_handler_str
        ))
        return False
...
def validate_consumers(config, logger):
    ...
    for consumer in config.get('consumers', []):
        if not is_consumer_valid(consumer, logger):
            is_valid = False
    return is_valid


def is_consumer_valid(consumer, logger):
    is_valid = True
    ...
    if not queue_name:
        logger.error('[{0}] No queue name specified'.format(
            consumer_str
        ))
        is_valid = False
    ...
    for key in ['prefetch_count', 'consumer_count']:
        ...
        if not isinstance(value, int):
            logger.error('[{0}] {1} must be an integer, found {2}'.format(
                consumer_str,
                key,
                value
            ))
